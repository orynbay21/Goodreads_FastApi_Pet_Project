...
def scraping():
    ...
    print("SCRAPED")
    return names, authors, publishers,img_urls[:-1]
...
#sending a generated pdf to a telegram channel
@dramatiq.actor(store_results=True)
def send_pdf(chat_id: str):
    token = os.environ.get("TG_API_TOKEN")
    names, authors, publishers, img_urls = scraping()
    data_arrays = list(zip(names, authors, publishers))
    image_paths = save_images(img_urls)
